...
"$COMMANDS_DIR"/exec.sh bash -c "if [ -f \"package.json.sample\" ] && [ ! -f \"package.json\" ]; then cp package.json.sample package.json; fi"
"$COMMANDS_DIR"/exec.sh bash -c "if [ -f \"grunt-config.json.sample\" ] && [ ! -f \"grunt-config.json\" ]; then cp grunt-config.json.sample grunt-config.json; fi"
"$COMMANDS_DIR"/exec.sh bash -c "if [ -f \"Gruntfile.js.sample\" ] && [ ! -f \"Gruntfile.js\" ]; then cp Gruntfile.js.sample Gruntfile.js; fi"

# NPM Install
"$COMMANDS_DIR"/exec.sh bash -c "npm install && npm update"

# Prepare local-themes file
"$COMMANDS_DIR"/exec.sh bash -c \
...
"$COMMANDS_DIR"/exec.sh bash -c "grunt exec:magento && grunt watch:magento"
