...
#
# The dashboard is the Go binary's, and this exists so that the command is one command.
#
...
# be downloaded — and in that case the honest answer is that there is nothing to serve, not
# "command not found".
#
...
if [ ! -x "$binary" ]; then
    hm_fail "$HM_EXIT_ERROR" "binary_missing" \
        "The dashboard is served by the ${COMMAND_BIN_NAME} binary, and it is not installed here" \
